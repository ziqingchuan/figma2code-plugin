...
flowchart TD
    ...
    Init --> LoadSettings[加载用户设置]
    LoadSettings --> ShowUI[显示 UI 界面]
    ...
    UIMessage -->|设置变更| UpdateSettings[更新设置]
    UIMessage -->|获取 JSON| ExportJSON[导出节点 JSON]
    
    UpdateSettings --> SaveSettings[保存设置]
    SaveSettings --> StartConversion
...
sequenceDiagram
    ...
    User->>Figma: 打开插件
    Figma->>Backend: 初始化插件
    Backend->>Backend: 加载用户设置
    Backend->>UI: 显示 UI 窗口
    Backend->>UI: postMessage(settingsChanged)
    UI->>UI: 更新设置状态
    ...
    User->>UI: 修改设置
    UI->>Backend: postMessage(pluginSettingWillChange)
    Backend->>Backend: 更新设置
    Backend->>Backend: 保存到 clientStorage
    Backend->>Backend: 重新执行转换
    Backend->>UI: postMessage(code, preview)
    UI->>UI: 更新显示
...
```mermaid
flowchart TD
    Start([插件启动]) --> LoadSettings[加载设置]
    LoadSettings --> ReadStorage[读取 clientStorage]
    
    ReadStorage --> CheckExists{设置存在?}
    CheckExists -->|否| UseDefaults[使用默认设置]
    CheckExists -->|是| ValidateSettings[验证设置]
    
    ValidateSettings --> MergeSettings[合并默认设置]
    UseDefaults --> MergeSettings
    
    MergeSettings --> ApplySettings[应用设置]
    ApplySettings --> NotifyUI[通知 UI]
    
    NotifyUI --> WaitForChange[等待设置变更]
    
    WaitForChange --> UserChange{用户修改设置?}
    UserChange -->|是| ReceiveChange[接收变更消息]
    UserChange -->|否| WaitForChange
    
    ReceiveChange --> UpdateInMemory[更新内存中的设置]
    UpdateInMemory --> SaveToStorage[保存到 clientStorage]
    SaveToStorage --> TriggerConversion[触发重新转换]
    
    TriggerConversion --> WaitForChange
```
